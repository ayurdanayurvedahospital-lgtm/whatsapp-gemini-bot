...
**Rules:**
1. **BILINGUAL CONTENT:** Provide **Benefits** AND **Usage** in **English AND Malayalam**.
2. **KEEP IT SHORT:** Your answer MUST be under 150 words.
3. **HIDDEN PRICE:** Do NOT mention price unless asked.
...
*** EXTENSIVE KNOWLEDGE BASE (OFFLINE STORES, Q&A, ETC.) INCLUDED ***
"""
...
def get_ai_reply(user_msg):
    ...
    for attempt in range(2): # Reduced retries for speed
        try:
            # Timeout set to 8 seconds to ensure we reply to WhatsApp in time
...
def bot():
    ...
    if step == "ask_name":
        ...
        ai_reply = get_ai_reply(f"Tell me about {incoming_msg} benefits and usage. English and Malayalam. Hide price.")
        
        # 🛡️ SAFETY 1: Remove double asterisks
        if ai_reply: ai_reply = ai_reply.replace("**", "*")
        
        # 🛡️ SAFETY 2: Limit length to 1000 chars (prevents huge messages)
        if len(ai_reply) > 1000: ai_reply = ai_reply[:1000] + "..."

        # 🛡️ SAFETY 3: Print AI Reply to logs for debugging
        ...
        if ai_reply: ai_reply = ai_reply.replace("**", "*")
        if len(ai_reply) > 1200: ai_reply = ai_reply[:1200] + "..."
        print(f"🤖 AI REPLY: {ai_reply}")
