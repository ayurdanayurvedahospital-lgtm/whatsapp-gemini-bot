...
FORM_FIELDS = {
    "name": "entry.2005620554",   # Will store "Name & Place" combined
    "age": "entry.1045781291",    # Will be left empty
    "place": "entry.942694214",   # Will be left empty (since it's in Name)
...
# --- SYSTEM INSTRUCTIONS (FULL BRAIN) ---
SYSTEM_PROMPT = """
...
def save_to_google_sheet(user_data):
    try:
        form_data = {
            FORM_FIELDS["name"]: user_data.get("name"),  # Stores "Rahul, Kochi"
            FORM_FIELDS["age"]: "",                      # Empty (Skipped)
            FORM_FIELDS["place"]: "",                    # Empty (Included in Name)
...
def bot():
    ...
    if sender_phone not in user_sessions:
        # Auto-capture Phone
        new_session = {
            "step": "ask_details",  # NEW STEP NAME
            "data": {
        ...
        msg.body("Namaste! Welcome to Alpha Ayurveda. 🙏\nTo better assist you, may I know your *Name and Place*?")
        return str(resp)
    ...
    # --- 🟢 2. COLLECT DETAILS (One Step Only) ---

    if step == "ask_details":
        session["data"]["name"] = incoming_msg # Saves "Rahul, Kochi"
        session["data"]["place"] = "" # Left blank in separate column
        session["data"]["age"] = "" # Skipped
